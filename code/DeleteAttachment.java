...
public class DeleteAttachment extends HttpServlet{
	...
	throws ServletException,IOException{
		...
		String temp_camp_id = req.getParameter("id");
		int camp_id;
		try{
		...
		try{
			...
			if(rs.next()){
				...
				unsubs=	rs.getInt("unsubs_reqd");
				file_name=rs.getString("attach_field");						
			}
	        //if(file_name!=null)
			 stmt.executeUpdate("update mail_campaign set attach_field = null where camp_id="+camp_id);
			//else
			 //out.println("There is no file attached with this campaign.<br><a href='javascript:history.go(-1)'>Click Here</a>  to go back to previous page ");
			 if(file_name!=null)
			{
				File file=new File(getServletContext().getRealPath("")+"/attachments/"+camp_id+"_"+file_name);
				file.delete();
			}	
			out.println("<html>");
			out.println("<head>");
		    ...
		    out.println("</tr>");	
		   /* out.println("<tr align='center'>");
		    out.println("<td width='100%' height='27' valign='top' align='left' colspan='2'><font face='Verdana' ><a href='DownloadAttachment?id="+camp_id+"&file="+file_name+"' target='_blank'>DownLoad Attachment</a></font></td>");
		    out.println("</tr>");
	        out.println("<tr align='center'>");
