...
public class EditList2 extends HttpServlet{
	...
	throws ServletException,IOException{
		...
		if((member_email==null)||(member_name ==null))
		{
		  out.println("<H2>Some field or fields may have been left empty</H2>");
		  out.println("<a href='javascript:history.go(-1)'>Click Here</a>  to go back to previous page & try again");
		  return;		
		}		
		if(member_email.equals("")||member_name.equals(""))
		{
		  out.println("<H2>Some field or fields may have been left empty</H2>");
		  out.println("<a href='javascript:history.go(-1)'>Click Here</a>  to go back to previous page & try again");
		  return;
		}
		if(checkmail(member_email));
        else{
		...
		try{
			...
			ResultSet rs;
			rs=stmt.executeQuery("select * from list_member where member_email='"+member_email+"' AND list_id="+list_id);
			if(rs.next()){
				out.println("<H2>Changed Email Id already Exists.</H2>");
			...
			stmt.executeUpdate("Update list_member set member_name='"+member_name+"',member_email='"+member_email+"' where list_id="+list_id+" AND member_email='"+email+"'");			
			out.println("<html>");
			out.println("<head>");
			...
			out.println("<tr>");
			out.println("<td width='100%' height='27' valign='top' align='left' colspan='2'><div align='center'><center><p><font face='Verdana' color='#000080'><small><br><br><br><br>Successfully Edited List<br><br></small></font></td>");
			out.println("</tr>");
