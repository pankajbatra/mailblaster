...
public class DeleteCamp2 extends HttpServlet{
	...
	throws ServletException,IOException{
		...
		try{
			...
			ResultSet rs;
			rs=stmt.executeQuery("select attach_field from mail_campaign where camp_id="+camp_id);
			while(rs.next())
			{
	    	...
	    	if(attach_file!=null)
			{
				File file=new File(getServletContext().getRealPath("")+"/attachments/"+camp_id+"_"+attach_file);
				file.delete();
			}	
			stmt.executeUpdate("Delete from mail_campaign where camp_id="+camp_id);
			stmt.executeUpdate("Delete from camp_list where camp_id="+camp_id);
			stmt.executeUpdate("Delete from camp_report where camp_id="+camp_id);
			stmt.executeUpdate("Delete from camp_links where camp_id="+camp_id);
			stmt.executeUpdate("Delete from camp_indi_report where camp_id="+camp_id);
			out.println("<html>");
			out.println("<head>");
			...
			out.println("<tr>");
			out.println("<td width='100%'><font face='Verdana' color='#FFFFFF'><small><strong>Deleted Campaign..</strong></small></font></td>");
			out.println("</tr>");
			out.println("</table>");
			out.println("</div><div align='right'>");
			...
			out.println("<td width='100%' height='27' valign='top' align='left' colspan='2'><font face='Verdana' color='#000080'><small><center><br>CampaignSuccessfully Deleted.<br><br></small></font></td>");
			out.println("</tr>");
			out.println("<tr align='center'>");
